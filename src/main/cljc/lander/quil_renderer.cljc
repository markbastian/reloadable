...
#_(defn draw-stats [{:keys [terrain lander]}]
  (let [{:keys [state theta fuel-mass] } lander
        [_ px py _ vy] state]
    (q/stroke 255)
    (q/fill 255)
    (q/text (str "Elevation: " (format "%5.2f" (double (- py (terrain/terrain-height px terrain))))) 0 15)
    (q/text (str "Rotation: " theta) 0 30)
    (q/text (str "Fuel: " (format "%5.2f" (double fuel-mass))) 0 45)
    (q/text (str "Y Velocity: " (format "%5.2f" (double vy))) 0 60)))

;Try purple [255 0 255]
(def thrust-color [255 0 0])
(def landing-zone-color [255 0 0])
...
(defn draw-lander [{{[_ px py] :state :keys [theta thrust width height]} :lander }]
  (let [tw (* 0.5 width) th (* 0.5 height)]
    ...
    ;Make true to always show
    (when (pos? thrust)
      (do (apply q/fill thrust-color)
          (apply q/stroke thrust-color)
          (q/push-matrix)
          (let []
            ;Add a multiplier (e.g. th -> (* 5 th)
            (q/translate 0 (- th))
...
(defn draw-landing-zones [{{ :keys [locations width] } :landing-zones terrain :terrain }]
  (apply q/stroke landing-zone-color)
  (apply q/fill landing-zone-color)
